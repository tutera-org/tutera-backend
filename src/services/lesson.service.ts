...
export class LessonService {
  ...
  async getLessonById(lessonId: string, tenantId: string) {
    const lesson = await LessonModel.findOne({ _id: lessonId, tenantId });
    if (!module) throw new AppError('Module not found', 404);
    return lesson;
  }

  async updateLesson(lessonId: string, data: Partial<LessonDTO>, tenantId: string) {
  ...
  async deleteLesson(lessonId: string, tenantId: string) {
    const lesson = await LessonModel.findOneAndDelete({ _id: lessonId, tenantId });
    if (!module) throw new AppError('Lesson not found', 404);
    return lesson;
  }

  async getLessonsByModule(moduleId: string, tenantId: string) {
